...
/** Ported from the team's inspo doc — used on the Solution section to flip between a plain-language term and the tech behind it. */
export function FlippingWordSwap({
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      const firstWord = gsap.utils.toArray<HTMLElement>('[data-flip-word="first"]');
      const secondWord = gsap.utils.toArray<HTMLElement>('[data-flip-word="second"]');

      gsap.set(firstWord, { rotationX: 0, opacity: 1, transformOrigin: "center top" });
      gsap.set(secondWord, { rotationX: -82, opacity: 0, transformOrigin: "center bottom" });
  ...
  const updateSwap = useCallback((next: boolean) => {
    ...
    setIsSwapped(next);
    if (next) timelineRef.current?.play();
